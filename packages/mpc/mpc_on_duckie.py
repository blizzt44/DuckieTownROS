...
class RunMPCNode(DTROS):
    """
    This is the monolith MPC node that will run on the Duckiebots
    This Node assumes a schedule already exists!
    """

    def __init__(self, node_name, schedule_path=None):
        super(RunMPCNode, self).__init__(node_name=node_name, node_type=NodeType.GENERIC)
        self._vehicle_name = os.environ['VEHICLE_NAME']
        self._publisher = rospy.Publisher('chatter', String, queue_size=10)

    def run_mpc(self, EnvFolder, naive_tracker=False, ignore_speed_ref=False):
        """
        Docstring for run_mpc
        
        :param self: Self
        :param EnvFolder: Describes the environment, FIXME currently hardcorded
        :param naive_tracker: False on Duckiebot
        :param ignore_speed_ref: ???
        """
        ...
        root_dir = os.getcwd()
        data_dir = os.path.join(root_dir,"packages","data", DATA_NAME)
        cnfg_dir = os.path.join(root_dir,"packages","config")
        rospy.loginfo(f"root: {root_dir}. data: {data_dir}, cnfg: {cnfg_dir}")
...
if __name__ == "__main__":
    runmpc = RunMPCNode('ros_mpc')
    runmpc.run_mpc(EnvFolder="SmallEnv") # FIXME Currently hardcoded to 3x3 grid
    rospy.spin()